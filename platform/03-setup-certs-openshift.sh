...
oc apply -f "${GIT_ROOT}/platform/vendor/cert-manager/release/cert-manager.yaml"
oc rollout status -n cert-manager deployment/cert-manager-cainjector
oc rollout status -n cert-manager deployment/cert-manager
oc rollout status -n cert-manager deployment/cert-manager-webhook

oc -n cert-manager apply -f "${GIT_ROOT}/platform/components/cert-manager/ca.yaml"
oc -n registry apply -f "${GIT_ROOT}/platform/components/cert-manager/registry.yaml"
oc -n spire apply -f "${GIT_ROOT}/platform/components/cert-manager/spire.yaml"

# wait for cert secrets to be created
wait_cert() {
  echo "Waiting for certificate $1 - $2..."
  while [ "$(oc -n "$1" get secret "$2" -o jsonpath='{.data.tls\.crt}' 2>/dev/null)" == "" ]; do
    sleep 1
  done
}
...
oc -n cert-manager get secret rootca -o jsonpath='{.data.ca\.crt}' | base64 -d >"${ca_cert}"
...
for ns in default registry tekton-chains tekton-pipelines; do
  oc -n "${ns}" create configmap ca-certs \
    --from-file=ca-certificates.crt="${ca_bundle}" \
    --dry-run=client -o=yaml | oc apply --server-side=true -f -
done

oc -n default create configmap java-certs \
  --from-file=cacerts="${ca_javacerts}" \
  --dry-run=client -o=yaml | oc apply --server-side=true -f -

oc -n vault create configmap ca-certs \
